...
class Rect:
    ...
    def transformed(self, t: NDArray[Any]) -> Rect:
        ...
        def transform(a: NDArray[Any]) -> NDArray[Any]:
            return t.dot(np.concatenate((a, [0] * extra_dimensions, [1]), axis=0))[
                : self.dimensions
            ]
    ...
    def bordered(self, border: float | NDArray[Any]) -> Rect:
        ...
        return Rect.from_mins_maxes(self.mins - border, self.maxes + border)

    def extended_to_integer_coordinates(self) -> Rect:
        """Return a rect that is expanded in all directions by the border."""
        return Rect.from_mins_maxes(np.floor(self.mins), np.ceil(self.maxes))
    ...
    def clamp_rectangle(self, rect: Rect) -> Rect:
        """Return the point or rectangle clamped to this rectangle."""
        return Rect.from_mins_maxes(
            np.minimum(self.mins, rect.mins), np.maximum(self.maxes, rect.maxes)
        )

    def rectified(self) -> Rect:
        """Fixes swaped min-max pairs."""
        return Rect.from_mins_maxes(
            np.minimum(self.mins, self.maxes), np.maximum(self.maxes, self.mins)
        )
    ...
    def intersection(self, other: Rect, /) -> Rect:
        ...
        return Rect.from_mins_maxes(
            np.maximum(self.mins, other.mins), np.minimum(self.maxes, other.maxes)
        )

    def union(self, other: Rect, /) -> Rect:
        """Return the smallest rectangle that contains both rectangles."""
        return Rect.from_mins_maxes(
            np.minimum(self.mins, other.mins), np.maximum(self.maxes, other.maxes)
        )

    def point_union(self, point: NDArray[Any], /) -> Rect:
        """Return the smallest rectangle that contains this rectangle and the point."""
        return Rect.from_mins_maxes(np.minimum(self.mins, point), np.maximum(self.maxes, point))
    ...
    def __contains__(self, point_or_rect: Rect | NDArray[Any], /) -> bool:
        if isinstance(point_or_rect, Rect):
            return bool(
                np.all(point_or_rect.mins >= self.mins and point_or_rect.maxes <= self.maxes)
            )
